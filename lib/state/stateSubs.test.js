...
describe('subscriptions', () => {
  ...
  test('subscribe', () => {
    ...
    expect(subs?.get(bindFn)?.[0]?.element).toBe(element)
    expect(subs?.get(bindFn)?.[0]?.elementProp).toBe(elementProp)
    expect(subs?.get(bindFn)?.[0]?.elementInnerProp).toBe(elementInnerProp)
    expect(subs?.get(bindFn)?.[0]?.repaintFn).toBe(repaintFn)
    ...
    expect(subs?.get(bindFnTwo)?.[0]?.element).toBe(element)
    expect(subs?.get(bindFnTwo)?.[0]?.elementProp).toBe(elementProp)
    expect(subs?.get(bindFnTwo)?.[0]?.elementInnerProp).toBe(elementInnerProp)
    expect(subs?.get(bindFnTwo)?.[0]?.repaintFn).toBe(repaintFn)
  ...
  test('migrateSubscriptions', () => {
    ...
    expect(subs?.size).toBe(1)
    expect(subs?.get(bindFn)?.[0]?.element).toBe(toElement)

    subs = getSubsBy(state, statePropTwo)
    expect(subs?.size).toBe(2)
    expect(subs?.get(bindFn)?.[0]?.element).toBe(toElement)
    expect(subs?.get(bindFnTwo)?.[0]?.element).toBe(toElement)
  })
  ...
  test('migrateSubscriptions bindFn', () => {
    ...
    expect(subs?.size).toBe(1)
    expect(subs?.get(bindFn)?.[0]?.element).toBe(fromElement)
    expect(subs?.get(bindFn)?.[0]?.elementProp).toBe('elementProp')

    subs = getSubsBy(state, statePropTwo)
    expect(subs?.size).toBe(2)
    expect(subs?.get(bindFn)?.[0]?.element).toBe(toElement)
    expect(subs?.get(bindFn)?.[0]?.elementProp).toBe('textContent')
    expect(subs?.get(bindFnTwo)?.[0]?.element).toBe(fromElement)
    expect(subs?.get(bindFnTwo)?.[0]?.elementProp).toBe('elementProp')
  })
})
