...
class StateProxy {
  ...
  #onPropUpdateInForState(prop, updatedObject, updatedState) {
    ...
    const parentOfUpdatedState = getParentState(updatedState)

    for (const [element, elementSubscriptions] of subscriptions) {
      for (const subscription of elementSubscriptions) {
        const parentState = getParentState(subscription.state)

        if (parentState && parentState !== parentOfUpdatedState) continue
