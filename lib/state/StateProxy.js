...
import { setElementAttrOrProp, setElementStyleRule } from '../functions/misc.js'
...
class StateProxy {
    ...
    set: (target, prop, value, receiver) => {
      if (
        ...
        if (
          ...
          const subscriptions = this.#subs.subscriptions.get(
            EnumSystemProps.reactive
          )

          if (subscriptions) {
            for (const [element, elementSubscriptions] of subscriptions) {
              for (const subscription of elementSubscriptions) {
                if (subscription.state === prevObject) {
                  subscription.state = receiver[prop]

                  for (const key in target[prop]) {
                    this.#onPropUpdateInForState(
                      key, target[prop], receiver[prop]
                    )
                  }
                }
              }
  ...
  #onArrayFunctionCallback = (action, updatedObject, updatedState, args) => {
    const subscriptions = this.#subs.subscriptions.get(EnumSystemProps.reactive)
    ...
    for (const [element, elementSubscriptions] of subscriptions) {
      for (const subscription of elementSubscriptions) {
        ...
        if (propertyName === 'style' && subPropertyName) {
          setElementStyleRule(element, subPropertyName, result)
        }
        else if (
          propertyName === EnumSystemProps.if
          || propertyName === EnumSystemProps.for
          || propertyName === EnumSystemProps.nest
        ) {
          if (repaintFunction) {
            // @ts-ignore
