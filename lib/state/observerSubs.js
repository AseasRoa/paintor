import { symObserverAccessProp } from '../common/constants.js'
import { getStateProps, isState } from './stateProps.js'
...
export function runCallbacks(type, target, state, key, oldValue) {
  ...
  while (true) {
    const subsForProp = getSubsBy(type, currentState, key)
    ...
    if (subsForProp) {
      ...
      for (const listener of subsForProp) {
        ...
        listener({ key, path, value, oldValue, state, target })
      }
    }

    const subsForUndefined = getSubsBy(type, currentState, undefined)
    ...
    currentState = stateProps?.parent

    if (!currentState) break

    if (stateProps?.key) {
      path.unshift(stateProps.key)
    }
  }
}
