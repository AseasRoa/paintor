...
import { escapeHTML } from './functions/string.js'
...
export class Element extends Node {
   ...
   * - If an attribute is redefined, its place in the order is
   *   where the initial definition is
   * - Name: If the attribute has no name, it's not painted
   * - Name: Attribute names are lower case
   * - Value: If boolean type, this is converted to string
   *   ("true" or "false")
   * - Value: If undefined or null, it is converted to empty
   *   string
   * - Value: If Array, the value is all values separated with
   *   commas
   * - Value: If Object, the value is [object Object]
   * - Value: If Boolean Attribute (hidden, disabled...), it
   *   doesn't have value
   *
  ...
  setAttribute(attributeName, attributeValue) {
     ...
     * Fix the value
     *
     * Nope, the value is not fixed here. Instead, the actual
     * value is preserved here.
     * It's fixed later on the paining stage, because a special
     * care is needed for Boolean Attributes
     */
