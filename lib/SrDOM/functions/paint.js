 ...
 * @returns {string}
 */
function paintHTMLElement(node, indentWith, indentCurrent, isChild = false) {
  let output = ''
  let indent = ''
  let newRow = ''
...
function paint(node, indentWith, indentCurrent, isChild = false) {
  switch (node.nodeType) {
    case EnumNodeTypes.ELEMENT_NODE: {
      ...
      return paintHTMLElement(node, indentWith, indentCurrent, isChild)
    }
 ...
 * @returns {string}
 */
function paintChildren(node, { indent = '' }) {
  // Paint the children
  let children = ''
  let childNumber = 0
