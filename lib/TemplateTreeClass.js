...
import { migrateSubscriptions, unsubscribe } from './state/subscriptions.js'
...
class TemplateTreeClass {
  ...
  createElement(tagName, ...args) {
    ...
    for (const argument of args) {
      ...
      else if (isObjectOrArray(argument) && argumentID === 1) {
        /*
   ...
   */
  #removeRenderedElements(commentElementBegin, commentElementEnd, prop) {
    ...
    if (prop === '*') {
       ...
       */
      if (
        commentElementBegin !== commentElementEnd
        && commentElementBegin.previousElementSibling === null
        && commentElementEnd.nextElementSibling === null
      ) {
        for (const key in keyToElements) {
          this.#removeRenderedElementsHelper(keyToElements, key, true)
        }
        const parentElement = commentElementBegin.parentElement

        if (parentElement) {
          parentElement.textContent = ''
          parentElement.appendChild(commentElementBegin)
          parentElement.appendChild(commentElementEnd)
        }
      }
      else {
        for (const key in keyToElements) {
          this.#removeRenderedElementsHelper(keyToElements, key)
        }
      }
      ...
      this.#removeRenderedElementsHelper(keyToElements, prop)
    }
  }
   ...
   * @param {ObjectKey} key
   * @param {boolean} [leaveDomElement]
   */
  #removeRenderedElementsHelper(keyToElements, key, leaveDomElement) {
    /** @type {Node[]} */
    ...
    for (const elementProp in properties) {
      ...
      if (typeof property === 'function') {
        ...
        if (property instanceof Array) {
          element[elementProp] = this.#translator.translateArray(property)
        }
        else {
          element[elementProp] = this.#translator.translate(property)
        }
      }
      else {
        if (
    ...
    const repaintFn = (
       ...
       */
        ...
        this.#removeRenderedElements(
          commentElementBegin, commentElementEnd, prop
        )
      }
        ...
        if (action === EnumStateAction.ARRAY_SPLICE) {
          ...
          if (keyToElements instanceof Array) {
            ...
            if (leftGroup.length > 0) {
              ...
              let leftBorderElement = commentElementBegin
            ...
            if (rightGroup.length > 0) {
              ...
              let rightBorderElement = commentElementEnd
            ...
            if (newLength < length) {
              if (newLength === 0) {
                ...
                while (index > 0) {
                  ...
                  this.#removeRenderedElements(
                    commentElementBegin, commentElementEnd, index.toString()
                  )
                }
              }
            }
  ...
  #unsubscribeElementAndItsChildren(element) {
    ...
    while (index > 0) {
      ...
      // @ts-ignore
      this.#unsubscribeElementAndItsChildren(childNodes[index])
    }
  }
}
