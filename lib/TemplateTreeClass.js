...
import { EnumStateAction, HTML_TAGS, symEmptyHandlerElements } from './constants.js'
...
import { setSuggestedItems, unsetSuggestedItems } from './state/elementsSuggestor.js'
import {
  moveSubscriptions,
...
/** @type {WeakMap<Comment, ObjectKey[]>} */
const mapKeysOrder = new WeakMap()
...
class TemplateTreeClass {
  ...
  if(condition, handler, elseHandler) {
    ...
    return (condition instanceof Function)
      ? this.#statementHandlerForFunction('if', condition, true, callbackForFunction)
