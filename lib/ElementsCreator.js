...
import { EnumStateAction, HTML_TAGS, symTemplateFunction } from './constants.js'
...
import { getStateProps, isState } from './state.js'
import {
  moveSubscriptions,
  removeAllSubscriptions,
} from './elementSubscriptions.js'
...
/**
 * This keeps maps of rendered DOM elements
 *
 * @type {WeakMap<Comment, RenderedElementsMap>}
 */
const renderedElementsMaps = new WeakMap()
...
class ElementsCreator {
  ...
  forState(input, handler, handlerOnEmpty) {
    ...
    if (stateProps) {
      ...
      const idx = stateProps.path.lastIndexOf('.')
      const keyToPaint = stateProps.path.substring(idx + 1)
  ...
  #removeRenderedElements(commentElementEnd, prop, isArray) {
    const reMap = renderedElementsMaps.get(commentElementEnd)

    if (!reMap) {
      return
    }

    let index = reMap.length
    ...
    while (index > 0) {
      ...
      if (
        prop === '*'
        || reMap[index]?.key === prop
      ) {
        const elements = reMap[index]?.elements

        if (!elements) continue

        for (const element of elements) {
          // @ts-ignore
          if (renderedElementsMaps.has(element)) { // inner end element
            // @ts-ignore
        ...
        if (isArray) {
          // @ts-ignore
          reMap[index].elements.length = 0

          delete reMap[index]
        }
        else {
          renderedElementsMaps.set(
            commentElementEnd,
            arrayRemoveKey(reMap, index)
          )
        }
      }
    }
  ...
  #statementHandlerForState(
    ...
    const repaintFunction = (
      ...
      if (
        this.#isSr
        || !(updatedObject instanceof Object)
      ) {
        return
      }
      ...
      if (action === EnumStateAction.CREATE) {
        ...
        if (updatedObject instanceof Array) {
          // @ts-ignore
          const prevIndex = parseInt(prop) - 1
       ...
       */
          ...
          if (reMap instanceof Array) {
            ...
            if (sizeDiff > 0) {
              ...
              for (
                ...
                // @ts-ignore
                reMap[index] = reMap[oldIndex]

                // @ts-ignore
                reMap[index].key = index.toString()

                delete reMap[oldIndex]
              }
            }
            else if (sizeDiff < 0) {
              reMap.splice(start, deleteCount)
            ...
            reMap.length = updatedObject.length
          }
        }
      }
        ...
        if (reMap instanceof Array) {
          // @ts-ignore
          const tmp = reMap[key2].elements

          // @ts-ignore
          reMap[key2].elements = reMap[key1].elements

          // @ts-ignore
          reMap[key1].elements = tmp
    ...
    renderedElementsMaps.set(commentElementEnd, added)
