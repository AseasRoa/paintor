...
import { cleanupTemplateQueue, isTemplate } from '../export/component.js'
...
import { HTML_TAGS, MUTUAL_TAGS, NAMESPACE_URI, SVG_TAGS } from './constants.js'
...
export class TemplateRenderer {
   ...
   */
  #cscPaintorClassNamesMap = new Map()
  ...
  $element(tagName, ...args) {
    const element = (tagName)
      ? (SVG_TAGS.has(tagName))
        ? this.#document.createElementNS(NAMESPACE_URI.SVG, tagName)
    ...
    for (let argument of args) {
      ...
      // @ts-expect-error
        ...
        let isTemplatesArray = false

        for (const child of array) {
          if (isTemplate(child)) {
            isTemplatesArray = true
        ...
        /*
         * Case 2) Array, containing at least one template
         * to be parsed and added to the element.
         */
        else if (isTemplatesArray) {
          for (const child of array) {
            if (!isTemplate(child)) {
              throw new Error(
                `While trying to create a new <${tagName}> element, mixed array was detected, containing at least one template, and other stuff. However, this is not supported. Use arrays containing only templates, only HTML elements, or neither templates of HTML elements.`
              )
            }

            this.#statementHandlerForTemplate(child, children)
          }
        }
        /*
         * Case 3) Array, containing string to be formatted.
         */
        else {
          const textNode = this.#document.createTextNode(
    ...
    if (this._cssClassName) {
      // To filter out SVGElement
      // @ts-expect-error
      if (element instanceof this.#window.HTMLElement) {
        // @ts-expect-error
        element.className = (element.className)
          // @ts-expect-error
          ? this._cssClassName + ' ' + element.className
          : this._cssClassName
      }
      else {
        // @ts-expect-error
        if (element instanceof this.#window.SVGElement) {
          // @ts-expect-error
      ...
      //     if (child.classList) {
      //       child.classList.remove(this._cssClassName)
      //     }
      //   }
      // }
    }
  ...
  $css(strings, ...keys) {
     ...
     */
      ...
      if (!this.#cssAdoptedStyleSheets.includes(strings)) {
        ...
        document.adoptedStyleSheets.push(strings)
      }
    }
  }
  ...
  #cssApplyString(string) {
    ...
    let className = this.#cscPaintorClassNamesMap.get(string)
    ...
    if (!className) {
      ...
      className = '--' + adoptedStyleSheetsCounter.toString()

      style.className = className

      /**
       * @see https://developer.mozilla.org/docs/Web/API/CSSStyleSheet/cssRules
       */
      ...
      this.#cscPaintorClassNamesMap.set(string, className)
  ...
  /**
   * @param {HTMLElement | SVGElement| Text} element
   * @param {(
   *   Object<
   *     string,
   *     (
   ...
   */
    ...
    for (const elementProp in properties) {
      ...
      if (typeof property === 'function') {
        ...
        // Check for Element instead of HTMLElement,
        // so it also works for SVGElement
        // @ts-expect-error
        && element instanceof this.#window.Element
      ) {
  ...
  #statementHandlerForTemplate(template, children) {
    ...
    const result = execTemplate(this.#thisTree, template)
    const generatedElements
      // @ts-expect-error
      = (result instanceof this.#window.HTMLElement)
        ? new Set([result])
  ...
  #statementHandlerResolver(handler) {
    if (typeof handler === 'function') {
      ...
      if (typeof result === 'function') {
        ...
        // @ts-expect-error
        resultResolver(this.#thisTree, result)
      }
    }
  }
...
const tags = [HTML_TAGS, SVG_TAGS, MUTUAL_TAGS]
