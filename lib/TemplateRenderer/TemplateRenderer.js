...
class TemplateRenderer {
  ...
  $css(strings, ...keys) {
     ...
     */
      ...
      this.#applyCssString(str)
    }
    else if (strings instanceof CSSStyleSheet) {
      if (!this.#adoptedStyleSheets.includes(strings)) {
    ...
    for (const argument of args) {
      ...
      if (typeof argument === 'string') {
        ...
        if (isTemplate(argument)) {
          ...
          /**
           * When executing the template, the newly created elements
           * are being appended to already existing set of children,
           * which could contain previous children. However, when
           * applying CSS stylesheets, we care only about the
           * newly created elements. That's why here we get
           * the current children size, so we can consider
           * this number later.
           *
           * @type {number}
           */
          const skipElements = children.size

          this.#statementHandlerForTemplate(template, children)
          this.#applyAdoptedStyleSheets(children, skipElements, false)
        }
        else {
          if (this.#isSr) {
            // @ts-expect-error
  ...
  async render() {
    ...
    for (const template of this.#templates) {
      if (typeof template === 'function') {
        const skipElements = this.getCreatedElements().size
        ...
        // @ts-expect-error
        this.#applyAdoptedStyleSheets(children, skipElements, true)
      }
      ...
      if (idx >= skipElements) {
        if (topLevel) {
          if (TAGS_SUPPORTING_SHADOW.has(element.tagName)) {
            for (const adoptedStyleSheet of this.#adoptedStyleSheets) {
          ...
          /**
           * Only the last sheet is applicable
           */
          const adoptedStyleSheet = this.#adoptedStyleSheets.at(-1)
          // @ts-expect-error
          const paintorClassName = adoptedStyleSheet?.paintorClassName

          if (paintorClassName) {
            element.className = paintorClassName + ' ' + element.className
          }
        }
      }
   ...
   * @param {string} string
   */
  #applyCssString(string) {
    if (!string) {
      return
    }

    let sheet = this.#adoptedStyleSheetsMap.get(string)
    ...
    if (!sheet) {
      ...
      this.#adoptedStyleSheetsMap.set(string, sheet)
    }

    if (!this.#adoptedStyleSheets.includes(sheet)) {
  ...
  #statementHandlerResolver(handler) {
    ...
    if (typeof handler === 'function') {
      ...
      // @ts-expect-error: We can throw any type to this function,
      // because it checks the input anyway, and returns nothing
      // if the input is wrong type
 ...
 */
prototype['if'] = prototype.$if
prototype['for'] = prototype.$repeat
prototype['forEach'] = prototype.$each
prototype['forState'] = prototype.$state
