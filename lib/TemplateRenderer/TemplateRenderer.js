...
class TemplateRenderer {
  ...
  $element(tagName, ...args) {
    ...
    if (this.#cssLastSheet) {
      /**
       * Add special class for
       */
      const paintorClassName = (level === 0)
        // @ts-expect-error
        ? this.#cssAdoptedStyleSheets.at(0)?.paintorClassName
        // @ts-expect-error
        : this.#cssLastSheet?.paintorClassName
