...
class TemplateRenderer {
  ...
  $element(tagName, ...args) {
    ...
    for (let argument of args) {
      ...
      if (typeof argument === 'string') {
        ...
        if (argument.css) {
          this.$css(argument.css.strings, argument.css.keys)
        }
        ...
        if (isTemplate(argument)) {
          ...
          this.#cssApplyAdoptedStyleSheets(children, skipElements, false)
        }
        else {
          if (this.#isSr) {
            // @ts-expect-error
  ...
  $if(condition, handler, elseHandler) {
    ...
    const elseHandlerToRun = ifUndefinedGetQueuedTemplate(elseHandler)
  ...
  async render() {
    ...
    for (const composable of this.#composables) {
      if (typeof composable === 'function') {
        ...
        let result = composable(this.#thisTree)
        ...
        // @ts-expect-error
        this.#cssApplyAdoptedStyleSheets(children, skipElements, true)
      }
    }
  ...
  #cssApplyAdoptedStyleSheets(
    ...
    const adoptedStyleSheet = (topLevel)
      ...
      : this.#cssAdoptedStyleSheets.at(-1)
    ...
    for (const element of elements) {
      // TODO: Find a way to not iterate over elements that should be skipped.
      //  The problem is, this loop would run frequently, each time skipping
  ...
  #statementHandlerResolver(handler) {
    if (typeof handler === 'function') {
      ...
      // 2.2) Normal Function, returning Template
      const result = runHandler(this.#thisTree, handler)
 ...
 */
prototype['createElement'] = prototype.$element
