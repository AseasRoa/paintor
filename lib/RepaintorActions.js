import { symEmptyHandlerElements } from './constants.js'
import { ElementsCollector } from './ElementsCollector.js'
import { toInt } from './functions/misc.js'
import { unsubscribeElementAndItsChildren } from './state/subscriptions.js'
...
class RepaintorActions {
      ...
      if (newLength < length) {
        if (newLength === 0) {
          this.#removeRenderedElements(
            this.#commentBegin, this.#commentEnd, '*'
          )
        }
        else {
          let index = length
  ...
  #createElementsHelper(elements, keyToElements, prop, lastElement) {
    ...
    for (const element of elements) {
      if (level === 0 && this.#collectedElements.length > level) {
        ...
        // @ts-ignore
        this.#collectedElements[level].moveElementAfterAnother(
          element,
          lastElement
        )
      }
   ...
   * @param {ObjectKey} prop
   */
  #removeRenderedElements(commentBegin, commentEnd, prop) {
